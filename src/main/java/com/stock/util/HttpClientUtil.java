...
public class HttpClientUtil {
    ...
    static {
        RequestConfig config = RequestConfig.custom().setConnectTimeout(60000).setSocketTimeout(1000 * 60).build();
    ...
    public static String doGet(String url) throws Exception {
        ...
        int responseCode = response.getStatusLine().getStatusCode();
        HttpEntity respEntity = response.getEntity();
        BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(respEntity.getContent(), "GBK"));
        String text;
        if (responseCode == HttpStatus.SC_OK || responseCode == HttpStatus.SC_BAD_REQUEST) {
            while ((text = bufferedReader.readLine()) != null) {
                stringBuilder.append(text);
    ...
    public Map<String, String> buildPostCommonParam(String methodName) throws Exception {
        String serialNo = RandomUtil.randomString(10);
        ...
        map.put("timestamp", DateUtils.dateToString(new Date(), "yyyy-MM-dd HH:mm:ss"));
        map.put("format", "json");
        map.put("v", "1.0");
        return map;
    }
    ...
    public String buildGetCommonParam(String methodName) throws Exception {
        ...
        sbuff.append("timestamp=").append(URLEncoder.encode(DateUtils.dateToString(new Date(), "yyyy-MM-dd HH:mm:ss"), CHARSET)).append("&");
